...
$(function() {
  ...
  $('#buyerInfoModal').on('show.bs.modal', function(e) {
    ...
    App.contracts.RealEstate.deployed().then(function(instance){
      ...
      $(e.currentTarget).find('#buyerAddress').text(buyerInfo[0]);//계정 주소
      $(e.currentTarget).find('#buyerName').text(web3.toUtf8(buyerInfo[1]));//계정 이름, 한글로 역변환
      $(e.currentTarget).find('#buyerAge').text(buyerInfo[2]);// 나이
    }).catch(function(err){
      console.log(err.message);
    })
